...
public class CharacterSelect22Action : MonoBehaviour
{
    // UI
    ...
    void Start()
    {
        ...
        if (PlayerPrefs.HasKey("player1Character")) {
            if (menuNo == PlayerPrefs.GetInt("player1Character")) {
                menuNo++;
            }
        }
    ...
    void Update() {
        ...
        */
        ...
        if (/*Input.GetButtonDown("Fire2") || */Input.GetKeyDown(KeyCode.Return)) {
            MyAudio.PlayOneShot(cursor);
            PlayerPrefs.SetInt("player2Character", menuNo);
            SceneManager.LoadScene("Main2"); // メニューへ
        ...
        if (Input.GetButtonDown("Cancel"/*"Jump"*/)) { // 
            ...
            SceneManager.LoadScene("CharacterSelect21"); // メニューへ
        }
